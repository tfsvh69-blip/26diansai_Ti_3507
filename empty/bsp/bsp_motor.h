 ...
 *
 * 【控制模型】每路一套梯形加减速状态机，由该路定时器的 ZERO 中断驱动：
 *   - STEP 由硬件定时器连续输出方波，步频 = 定时器时钟(4MHz) / 定时器周期；
 *   - 直接高速起转会失步，故从 500Hz 慢速起步、逐脉冲加速到目标转速、末段再减速；
 *   - 全部接口非阻塞：调用后立即返回，实际加减速在各自 ISR 后台完成。
 *
 * 【两个核心接口】（对应"指定速度/方向" 与 "指定脉冲/距离"）
 *   BspMotor_SetSpeedRpm(id, rpm)      连续旋转：rpm 正=正转/负=反转/0=平滑停止
...
/* 上电安全态：禁用四路驱动、四路 STEP 停表、DIR 归正向、默认 1/32 细分。 */
 ...
 * 默认全部 false。invert 只改变 DIR 电平方向，不影响 rpm/steps 的数值含义。
 */
...
/*
 * (a) 指定速度和方向（连续旋转，非阻塞）：
 *     rpm > 0  正转、rpm < 0  反转、rpm == 0  平滑减速停止。
 *     - 电机已停：设好方向后从慢速起步，ISR 平滑加速到该转速；
 *     - 同向运行中：等同在线调速，平滑过渡到新转速；
 *     - 反向运行中：先平滑减速停稳，再自动翻向重新加速（全程非阻塞）。
...
/* 平滑减速停止指定电机（等价 SetSpeedRpm(id,0)）。 */
void BspMotor_Stop(BspMotorId_t id);

/* 平滑减速停止全部四路。 */
void BspMotor_StopAll(void);

/* 立即急停指定电机（不减速，直接停表）；仅用于异常保护/复位，正常停车用 Stop。 */
