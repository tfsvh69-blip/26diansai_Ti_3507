...
- **状态显示**：OLED 菜单/运行界面底部状态栏最前显示 `R:ON `/`R:OFF`（继电器逻辑状态，两者等宽以免后面字段左右跳动）。测试任务每 2s 切换时，OLED 显示与继电器实际吸合/释放同步（已实测对上）。日后若换低电平触发模块，改 `bsp_relay.c` 的 `BSP_RELAY_ACTIVE_LOW=1` 即可让显示与动作重新对齐。
- 测试任务 `RELAYTEST`（`APP_FEATURE_RELAY` 门控，默认启用）：每 **2 秒**切换一次吸合/断开，用于验证继电器及负载。⚠️ 每次切换都会真实通断电磁铁大电流负载，测试时确保负载侧安全；不需要时把 `APP_FEATURE_RELAY` 置 0 即停任务（PA24 保持上电断开态）。
