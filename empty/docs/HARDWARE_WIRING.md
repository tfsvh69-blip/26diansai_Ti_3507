...
| ID1 归零限位开关 | HOME_SWITCH | PA24 | `HOME_SWITCH_PIN` | `IOMUX_PINCM54`，P1-3 | GPIO 数字输入 + 内部上拉；轻触开关一端接地，按下=低电平（原继电器接口，2026-08-01 改接） | 开机 `AppLiftHoming_RunAtBoot()` 自动归零 ID1，串口输出 `HOMING: ...` |
...
## ID1 归零限位开关 HOME_SWITCH（PA24，接口 P1-3，2026-08-01 由继电器改接）

> P1 接口原为继电器（v1.1 接线文档述 `RELAY ← PA24`，驱动大电流电磁铁负载）。
> 该功能已不再需要，接口上改接一颗轻触开关，作为 ID1（摆杆升降丝杆）的开机
> 归零限位开关；继电器相关代码（`bsp_relay.c/h`、`app_relay_test_task.c/h`、
> `APP_FEATURE_RELAY`/`APP_FEATURE_RELAY_SELFTEST`、OLED 状态栏 `R:ON/OFF`）
> 已整体删除。⚠️ **接线文档风险 R7 已随之解除**（原风险是继电器上电前高阻误
> 吸合；现在是普通输入开关，无此风险）。

- 硬件：轻触开关一端接地，另一端接 PA24；MCU 侧配为数字输入 + 内部上拉 + 迟滞（与 KEY1~4 同一配置方式）。按下时引脚被拉低。
- 封装接口 `bsp/bsp_home_switch.h`：`BspHomeSwitch_IsPressed()` 返回当前是否按下（低电平=按下），无需初始化。
- **用途**：`App_Init()` 在 `Emm42Robot_Init()` 之后、调度器启动前调用 `app/app_lift_homing.c` 的 `AppLiftHoming_RunAtBoot()`（由 `APP_FEATURE_LIFT_HOMING` 门控），驱动 ID1：
  1. 正方向移动直到压下限位开关；
  2. 压下瞬间反向（负方向）退让直到开关释放——释放点即物理归零参考点；
  3. 继续负方向移动 `LIFT_HOMING_TARGET_OFFSET_PULSES`（当前占位为 `0`，待实测填入）个脉冲到达指定工作位置；
  4. 若开机时开关已被压住，跳过第 1 步直接退让，避免继续顶向硬限位。
- 全程用 `Delay_ms` 忙等（调度器尚未启动），不清零位置——`BALLCTRL`/`task6` 各自的清零逻辑不变；这套流程取代了此前"每次上电先人工把杆摆到目视水平"的步骤。详见 `CLAUDE.md`「ID1 开机自动归零」小节。
- ⚠️ 归零流程故意不加超时保护：若限位开关故障导致第 1 步永远读不到触发，调度器不会启动（LED1 不闪），需要现场排查开关接线。
