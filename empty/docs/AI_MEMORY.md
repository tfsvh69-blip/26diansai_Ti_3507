...
- 【2026-07-26 NRF24L01+ 连续发射测试与诊断】板端接线固定为 CE=PA22、CSN=PA1（外部4.7kΩ上拉3V3）、SCK=PA27、MOSI=PA14、MISO=PA0，IRQ不接；五个信号脚均已按 v1.1 权威接线文档确认属于剩余可用引脚。USB 无线串口 V2.0 上位机地址输入按十六进制解释，当前地址 `{0x15,0x52,0x33,0x54,0x55}`，无线参数 2.402GHz/2Mbps/8位CRC。厂家透传协议必须固定发送32字节，`payload[0]`=有效正文长度1~31、正文从`payload[1]`开始。`NRF24TX`任务默认每500ms发送递增文本，IRQ不接而轮询STATUS，单次发送有12ms超时并可掉线重初始化。全局 `volatile Nrf24Diag_t g_nrf24Diag` 保存任务心跳、SPI计数、配置回读、地址、GPIO电平和发送结果，UART0 115200 默认每秒输出 `NRF D ...`；无线串口 COM 仍为 9600，二者不要混淆。

- 【2026-07-26 NRF24L01+ 无 ACK 兼容扫描】已实测板端 `FAIL=0`、配置读回正确、`MAX_RT` 每包触发且自动重传次数为10，说明 MCU-SPI-寄存器-CE 发射流程正常，但未收到 USB 端 ACK。`APP_NRF24_DIAG_COMPAT_SCAN=1` 时自动扫描 P0~P5（正/反地址，8/16 位 CRC，十进制地址解释），每组8包；首个 ACK 会置 `g_nrf24Diag.profileLocked=1` 锁定。扫描仍完全无 ACK 时，故障重点转向 USB 转接端接收/配置或板端发射供电、射频硬件，而非载荷文本协议。
