...
| `MOTOR1` | `app/app_motor_test_task.c` | 单次执行后挂起 | `APP_MOTOR_TEST_TASK_PRIORITY` | `APP_MOTOR_TEST_TASK_STACK_WORDS` | 无 | TIMG0_CCP0(PB10) 20kHz STEP、TIMG0 ZERO 中断计步 1600 脉冲、PB11 DIR、PA13 ENN，UART0 打印状态 | 电机1一圈测试：物理最小分频(prescale=0, 4MHz)，20kHz 步频，正向旋转一整圈(≈80ms)后停止并禁用 ENN，任务挂起 |
