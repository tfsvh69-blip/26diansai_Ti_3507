...
| `UIMENU` | `app/app_ui_task.c` | 30ms 按键轮询 | `APP_UI_TASK_PRIORITY` | `APP_UI_TASK_STACK_WORDS` | KEY1~4(PA28/PA31/PA30/PA29) 按下沿；**IMU Yaw 快照**(`AppImuUartTask_GetYaw`，⚠️`APP_FEATURE_IMU=0` 时恒返回 false)、**激光距离**(`LaserLd14_GetLatest`，⚠️`APP_FEATURE_LASER=0` 时恒返回 false)、**小球检测**(`BallParser_GetLatest`) | OLED(PB8/PB9 软件I2C) 题目菜单/运行界面 + 蜂鸣器(PA15)短促嘀声(~2~3ms) + 底部传感器状态栏 + 右侧小球面板 | **OLED 题目菜单 UI**：任一按键按下沿、题目二到达终点或题目五自动停车完成，均调用同一个短促提示音；菜单态 K1上移/K2下移(循环)、K3确认进入运行界面，K4 在菜单态无功能（原"启停钢球居中到 X=320"的调试入口已于 2026-08-01 移除，PA24 引脚已改接 ID1 归零限位开关）；运行态 K4 返回菜单。当前 5 道题均为硬件/函数测试（非正式赛题）：题目一 `VIDEO 5S` 通过视觉协议录制 5 秒无叠加标注的正常画面、题目二 `LINE PID` 正式循迹、题目三 `Task 3`（2026-08 由题目四整体移植，是题目四框架的完整副本，独立参数组）、题目四 `LINE 6S` 两段式 K3 启动（第一次进钢珠位置闭环，第二次才发车循迹+开始录像）并在累计前进设定时长后缓停、题目五 `Five` **2026-08 同样改为两段式 K3 启动**（第一次进钢珠位置闭环，第二次才发车循迹+开始录像），武装后命中 ≥5 路黑线判定到达终点，两轮改为同一转速直行保持 1500ms 后转入缓停，蜂鸣器第二次响/结束录像在其中更早的 800ms 先触发，再用与起步同一根软件斜坡对称降速到 0、题目六 `ID1 POS` 对 Emm42 ID1 做位置模式 API 冒烟测试。底部常驻**传感器/系统状态栏** `Y:<yaw> D:<dist>mm`(局部低频刷；IMU/激光关闭时 Yaw/D 显示 `---`)；启用 `APP_FEATURE_BALL_VISION` 时菜单右半常驻**小球检测文字面板** `BALL`/`F/n/x/y`(局部低频刷)。**独占 OLED 与 4 按键**，界面整屏刷为事件驱动 |
| `BALLCTRL` | `app/app_ball_control_task.c` | 10ms 轮询；仅在视觉新样本到达时才计算 | `APP_BALL_CONTROL_TASK_PRIORITY` | `APP_BALL_CONTROL_TASK_STACK_WORDS` | 长度 1 覆盖队列的启停/目标 X/profile；视觉 `xSeq/xPixel` | Emm42 ID1 **绝对位置模式**；闭环状态快照（`commandPulse`） | **默认启用但上电 OFF**。各题目经 `AppBallControl_RequestTargetWithProfile()` 按需启停各自目标（菜单已不提供 K4 启停入口，见「OLED 题目菜单 UI」小节）。**位置模式**：`pulseDelta = ffDir×frictionFfPulse + (Kx×error − Kv×v_filter)` 纯 PD（无 I 项，`ffDir` 是库仑摩擦前馈方向，`frictionFfPulse=0` 时自动退化为纯 PD），每帧下发绝对目标脉冲 `targetPulse = LEVEL_TRIM_PULSE + SIGN×pulseDelta`。`settleDeadbandPx` 是到位保持区：误差进死区【且】球基本停住（`|v|≤ffVelBlendPxps`）才回真实水平点、停止驱动，只有超出该范围才重新驱动。使能完成后进入 `ZERO` 状态，**只在本次上电后第一次启动时**发位置清零（`hasZeroedSinceBoot` 函数级 static），之后反复进出各题目调参沿用同一原点，重新上电才建立新原点（配合 ID1 开机自动归零，第一次清零的位置就是归零终点，见「ID1 开机自动归零」章节）。单帧 NA 不再改变状态；连续两帧 NA 或 220ms 无有效 X 才判定 `LOST`，**仅影响 OLED 显示，不下发任何停止命令**（摆杆保持在最后一次有效目标，位置模式本身安全）；唯一仍主动下发命令的保护是画面边缘（命令回水平并锁定 `FAULT_EDGE`，等题目自行发起恢复握手）。**2026-08 新增**：profile 含 `maxPulseStepPerFrame`（软件限速，0=不限速，菜单/题目三默认不变）；`BALL_CTRL_DEBUG_LOG_ENABLE`（默认 0）打开后把关键量打到 UART0（TX 空闲不冲突视觉 RX），排查完关闭。详见 `BALL_CONTROL.md`、`CONTROL_ALGORITHM.md` §10 |
...
- **两态状态机**：
  ...
  - `RUN` 运行态：大字 `TASK n` + 题名 + `K4: back to menu`；周期调用该题 `onLoop` 钩子。题目四、五两段式 K3 启动期间题名行显示 `T4/T5 K3=BALL`（等第一次 K3）、`T4/T5 B WAIT`（球杆闭环接管中）、`T4/T5 K3=GO`（等第二次 K3 发车）；发车后题目五题名行显示 `ARM`、`LINE`、`GO:<剩余秒数>`（终点线后直行保持，倒数到 1500ms）、`DEC`（对称软件斜坡缓停）或 `DONE`，便于观察终点线后的停车流程。
- **按键（30ms 轮询，去抖 + 按下沿）**：
...
- **题目表**：题名/题数登记在 `app_robot_core.c` 的 `s_robotTasks[]`（题目一 `VIDEO 5S`、题目二 `LINE PID`、题目三 `Task 3`、题目四 `LINE 6S`、题目五 `Five`、题目六 `ID1 POS`）；**各题业务钩子 `OnEnter/OnLoop/OnExit` 实现在 [`../app/tasks/`](../app/tasks/) 的 `taskN.c`**。题目一不驱动电机，向视觉端发起一次录像并在约 5 秒后自动停止，保存无叠加标注的正常画面；题目二、四均按 8 路灰度传感器计算偏差并 PID 调节 Emm42 的 ID2 左轮、ID3 右轮速度；题目四两段式 K3 启动，第二次按键才发车，同时全程通过 ID1 把钢珠保持在 `T4_BALL_TARGET_X_PX`，正常循迹累计前进 `T4_STOP_AFTER_MS` 后执行带加速度的速度模式 0 RPM 缓停。题目三现为题目四框架的完整副本（独立参数组）。UIMENU 只负责显示与按键，经 robot_core 分发调用；运行态 K3 按下沿另经 `RobotCore_ConfirmTask()` 转发给题目的 `onConfirm` 钩子（当前题目四、五登记，均为两段式发车启动）。
- **参数隔离**：同一算法可以在不同任务复用，但每个任务必须拥有独立可调参数宏和 profile，禁止跨任务直接共用增益、死区、滤波、静摩擦、速度或保持条件。任务三使用 `T3_*`（含钢珠闭环的 `T3_BALL_*`），任务四使用 `T4_*`/`T4_BALL_*`，任务五使用 `T5_*`/`T5_BALL_*`；三题参数结构大体一致（题目三、五分别是题目四框架的副本/改造）但数值各自独立；新增任务必须在自身 `taskN.c` 另建参数组。
...
- 终点检测：先连续 `T2_FINISH_ARM_TICKS` 拍命中路数 `≤T2_FINISH_ARM_HIT_MAX` 才"武装"终点检测（避免出发瞬间仍压在宽线上被立即误判为跑完一圈）；武装后再连续 `T2_FINISH_HIT_TICKS` 拍命中路数 `≥T2_FINISH_HIT_MIN`（6/7/8 路均算）**且累计时间 `≥T2_FINISH_MIN_ELAPSED_MS`（2026-08 新增，当前 3000ms）**，判定到达终点，立即硬停车（`Emm42Robot_Stop`）并锁定在 `T2_STATE_FINISHED`，之后不再重新进入循迹，等待 K4 手动退出；停车后调用 `BspBuzzer_BeepShort()`，响铃长度与按键反馈一致。**时间下限的动机**：赛道是环形，起跑线和终点线是同一根线，仅凭"武装+命中路数"防不住"赛道较小、发车不久又经过这根线"被误判成终点；题目三 `T3_FINISH_MIN_ELAPSED_MS`、题目五 `T5_FINISH_MIN_ELAPSED_MS` 同一动机、各自独立参数。
...
- **终点判定时间下限（2026-08 新增，与题目四不同）**：赛道是环形，起跑线和终点线是同一根线，仅凭"武装+命中路数 `≥T3_FINISH_HIT_MIN`"防不住发车不久又经过这根线被误判成终点；因此额外要求累计时间 `≥T3_FINISH_MIN_ELAPSED_MS`（当前 3000ms，独立参数，用原始 tick 计数不叠加 `T3_STOPWATCH_CAL_SCALE` 显示校准）才判定到达终点。题目二 `T2_FINISH_MIN_ELAPSED_MS`、题目五 `T5_FINISH_MIN_ELAPSED_MS` 同一动机、各自独立参数；题目四暂未加同款下限（其终止以 `T4_STOP_AFTER_MS` 定时缓停为主，`FINISH_HIT_MIN` 只是提前到达终点时的次要保护）。
...
- **录像开始时机同步延后到第二次 K3**：`app_robot_core.c` 的题目表新增 `deferVideoStart` 标记，只有题目四为 `true`——`RobotCore_EnterTask()` 对这类题目跳过自动的 `$TASK...START`，改由题目四在第二次 K3（车辆真正开始动作）时自己调用 `RobotCore_NotifyTaskStarted(3U)` 发送。结束仍是原有路径：缓停或终点保护触发蜂鸣器的同一处调用 `RobotCore_NotifyTaskFinished(3U)` 发送 `STOP`，未受影响。其余题目 `deferVideoStart=false`，进题目即自动开始录像，行为不变。若用户在第二次 K3 之前就按 K4 退出，`AppVisionLink_TaskStop()` 内部会因为没有对应的 `activeTaskId` 而自动跳过发送（不会产生无对应 START 的 STOP 帧）。
...
- **手动调参开关 `T4_MOTORS_DISABLED_MANUAL_TEST`**（`task4.c` 顶部）：置 1 时轮子不使能、不驱动，第二次 K3 也不会发车，只让后台 `BALLCTRL` 按 `T4_BALL_*` profile 保持钢球在目标位置，供用手推小车模拟前进/加减速扰动、专门调 `T4_BALL_*` 参数（OLED 显示 `T4 MANUAL`）；置 0（当前默认）才会执行完整两段式启动+循迹+缓停流程。

**题目五 `Five`**（`task5.c`，2026-08 由题目四整体框架改造：两段式 K3 启动 + 循迹到终点线 5 路判定 + 直行保持 + 对称软件斜坡缓停）：
- 硬件：UART1（PA17=TX → 驱动器 RX，PB5=RX ← 驱动器 TX）。**与旧版不同，本题现在同时使用 ID1、ID2、ID3**：ID1 摆杆由 `BALLCTRL` 独立线程按题目五私有 `T5_BALL_*` profile 闭环，ID2 左轮/ID3 右轮做循迹。
- **两段式 K3 启动**（与题目四同一套做法，`Task5_OnConfirm()` 经 `RobotCore_ConfirmTask()` 转发）：进题目后什么都不动（OLED 显示 `T5 K3=BALL`）；**第一次 K3** 向 `BALLCTRL` 投递 `T5_BALL_*` profile、目标 `T5_BALL_TARGET_X_PX`，等后台真正进入闭环（`RUNNING`/`HOLDING`，OLED 显示 `T5 B WAIT` → `T5 K3=GO`）；**第二次 K3** 才走 ID2/ID3 使能时序（复用题目二的节拍化状态机：先失能清残留状态，再依次使能两路，各等 `T5_ENABLE_SETTLE_TICKS` 拍）、秒表开始计时，并调用 `RobotCore_NotifyTaskStarted(4U)` 通知视觉端开始录像（题目表 `deferVideoStart=true`）。行驶、终点保持和缓停期间 ID1 均保持闭环；`BALLCTRL` 触发 `FAULT_EDGE`（球触边）时走与题目四相同的"停止→等释放→重新请求"自动恢复握手（`T5_STATE_BALL_RECOVER_WAIT`），恢复后若本次已判定到达终点（`s_runCompleted`）则直接停在 `T5_STATE_FINISHED`，不会被误判成还要继续循迹。
- **PID、加速度、顶速**：转向 PID 增益（`T5_KP/KI/KD`）与题目四当前值相同；基础速度 `T5_BASE_RPM`、单轮限幅 `T5_MIN_WHEEL_RPM`/`T5_MAX_WHEEL_RPM`/`T5_MIN_BASE_RPM`、转向输出限幅 `T5_MAX_STEER_RPM`、驱动器曲线档位 `T5_EMM_ACC` 均按用户要求"加速度和顶速参考题目四"直接复制题目四当前值（不再是旧版的 62% 缩放）。误差滤波/死区/转弯减速沿用题目二同款做法。
- **转向输出软件限速**（题目二/四没有，题目五独有）：`Task5_UpdateTracking()` 里 `Pid_Update()` 算出的原始转向量存入 `s_lastSteerRpm` 后，只作为"目标"；`s_appliedSteerRpm` 每拍最多向该目标靠近 `T5_STEER_SLEW_RPM_PER_SEC×T5_DT_SEC`，`Task5_ApplyNormalTracking()`/`Task5_ApplyDecelTracking()` 计算左右轮差速时改用限速后的 `s_appliedSteerRpm`（不再直接用 `s_lastSteerRpm`）。用于抑制离散灰度传感器命中路数切换带来的台阶式转向量一拍内跳到接近 `T5_MAX_STEER_RPM`、车身瞬间大幅横摆把摆杆上的钢珠带得晃动（2026-08 实车反馈"过弯 P 偏大、横摆导致球晃"后新增）。判定到达终点转入 `T5_STATE_AFTER_LINE` 时 `s_lastSteerRpm`/`s_appliedSteerRpm` 都清零，缓停阶段重新进入循迹后从 0 平滑爬升，不带旧转向残留。`T5_STEER_SLEW_RPM_PER_SEC` 当前值是新增功能第一版起点，需要现场按"横摆是否消失"/"过弯会不会跟不上"调整，不代表已验证的最终值。
- **起步/缓停共用同一根软件斜坡** `T5_RAMP_RPM_PER_SEC`（复制题目四的 `T4_START_RAMP_RPM_PER_SEC` 当前值）：发车时从 0 每拍向上抬到 `T5_BASE_RPM`，压住驱动器 acc 曲线本身压不住的起步阶跃；到终点线后的缓停阶段从当前值原样每拍向下压到 0——用户明确要求"跟小车从零加速一样的加速度来减速"，因此不另设独立的固定减速时长参数（这点与旧版 `T5_DECEL_MS` 固定 2 秒的线性缓停不同）。
- **终点判定**（与题目二/四系不同）：先连续 `T5_ARM_TICKS` 拍命中 `≤T5_ARM_HIT_MAX` 路细线，确认已离开出发线才"武装"；武装后单拍命中 `≥T5_STOP_LINE_HIT_MIN`（**当前 5，即 8 路里的 5 路**，用户本次明确给出的新阈值，区别于题目二/四系的 6 路）**且发车（第二次 K3）后累计时间 `≥T5_FINISH_MIN_ELAPSED_MS`（2026-08 新增，当前 3000ms，用原始 tick 计数不叠加 `T5_STOPWATCH_CAL_SCALE` 显示校准）**才判定到达终点，记录当前左右轮平均转速为 `s_straightRpm`，转入 `T5_STATE_AFTER_LINE`。**时间下限的动机**：赛道是环形，起跑线和终点线是同一根线，仅凭"武装+命中路数"防不住发车不久又经过这根线被误判成终点；题目二 `T2_FINISH_MIN_ELAPSED_MS`、题目三 `T3_FINISH_MIN_ELAPSED_MS` 同一动机、各自独立参数。
- **终点后直行保持**：`T5_AFTER_LINE_MS`（当前 1500ms）期间左右轮都按 `s_straightRpm` 直接下发（不再有转向修正），即"保持当前速度直直地前进"，不重新计算 PID、不减速。保持期间的 `T5_BEEP_DELAY_MS`（当前 800ms，早于 1500ms 走完）时刻：蜂鸣器响第二声（`BspBuzzer_BeepShort()`，`s_finishBeeped` 保证只响一次）、同时调用 `RobotCore_NotifyTaskFinished(4U)` 结束本次录像——录像/计分只覆盖到终点线 + 800ms，车辆之后仍按同一转速继续直行到满 1500ms 才转入 `T5_STATE_DECEL`，给机械留出裕量。
- **对称软件斜坡缓停**：`T5_RAMP_RPM_PER_SEC` 每拍从当前值向下压，同时继续跑循迹 PID 修正（`Task5_ApplyDecelTracking()`，差速限幅随当前基础速度同步收窄，保证两轮非负且最终归零）；斜坡降到 0 后再分两拍给 ID2、ID3 发送速度模式 0 RPM（`Emm42Robot_VelControl(..., 0, T5_STOP_EMM_ACC)`）收尾，转入 `T5_STATE_FINISHED`。长期丢线时（`T5_LINE_LOST_TICKS` 拍无信号）直接置零轮速，不再继续按斜坡衰减。
- 题目五秒表从发车（第二次 K3）起每 30ms 累加，两段式启动的三个等待态、故障恢复态和 `T5_STATE_FINISHED` 均不计时；`Task5_GetUiStatus()` 在 OLED 显示 `T:12.3s` 加当前阶段（`ARM`/`LINE`/`LOST`/`GO:1.2s`/`DEC`/`STOP`/`DONE`），显示时间乘 `T5_STOPWATCH_CAL_SCALE=0.897`，与任务二/四使用同一实测初值，可独立按"新系数 = 当前系数 × 实测时长 / 显示时长"继续校正。
- K4 退出仍走硬安全收尾：急停后失能 ID2/ID3，并调用 `AppBallControl_RequestStop()` 释放 ID1。
...
**2026-08 由题目四整体移植后，本节描述的"第一阶段 `X=325→415±10px` 定位"（`T3_STAGE1_*` 参数组、`T3_TO415/T3_STABLE/T3_TIMEOUT` 状态显示）已全部废弃**，不要再按本节理解题目三。题目三当前的准确行为见前文「题目三 `Task 3`」小节：它现在是题目四框架的完整副本，独立 `T3_*` 参数组，跟题目四一样有两段式 K3 启动、循迹、缓停等完整业务。
...
- **只做登记 + 分发**：`app_robot_core.c` 里的 **dispatch 表 `s_robotTasks[]`** 登记 6 道题的 name + `OnEnter/OnLoop/OnExit` 函数指针；**各题业务代码在 [`../app/tasks/`](../app/tasks/) 的 `taskN.c`**（第 N 题 = `taskN.c`，每题一套状态机；当前 task1 为约 5 秒正常画面录像，task2 为正式循迹，task3 是 task4 框架的完整副本（独立参数组），task4 为两段式 K3 启动的钢珠平衡+循迹定时缓停，task5 按说明实现，task6 `ID1 POS` 为 Emm42 ID1 位置模式 API 冒烟测试）。
