...
- `ZERO` 调 `Emm42Robot_ResetPosToZero()`（协议 `0x0A 0x6D`）把当前机械位置定义为绝对位置 0。**摆杆无限位开关/角度传感器，必须先人工把杆摆到目视水平再按 K3 进本题**；驱动器断电不保留多圈位置计数，每次上电都要重做。
