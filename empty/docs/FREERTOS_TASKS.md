...
- **按键（30ms 轮询，去抖 + 按下沿）**：
  ...
  - `K4`(PA29)：运行态返回菜单（先调 `onExit`）；菜单态首次请求默认 `BALL_CTRL_*` 参数的 X=350 单点验证，右侧 `X:`/`B:` 反馈实测位置与闭环状态；闭环运行时再次按下安全停止。
- **题目表**：题名/题数登记在 `app_robot_core.c` 的 `s_robotTasks[]`（题目一 `VIDEO 5S`、题目二 `LINE PID`、题目三 `BALL SWING`、题目四 `LINE 6S`、题目五 `Five`、题目六 `ID1 POS`）；**各题业务钩子 `OnEnter/OnLoop/OnExit` 实现在 [`../app/tasks/`](../app/tasks/) 的 `taskN.c`**。题目一不驱动电机，向视觉端发起一次录像并在约 5 秒后自动停止，保存无叠加标注的正常画面；题目二、四均按 8 路灰度传感器计算偏差并 PID 调节 Emm42 的 ID2 左轮、ID3 右轮速度；题目四两段式 K3 启动，第二次按键才发车，同时全程通过 ID1 把钢珠保持在 `T4_BALL_TARGET_X_PX`，正常循迹累计前进 `T4_STOP_AFTER_MS` 后执行带加速度的速度模式 0 RPM 缓停。题目三在限位回零并抬升后自动以 X=350 启动钢球闭环并等待 K3，按键才启动单次左右摆球，ID2/ID3 仅使能 0 RPM。UIMENU 只负责显示与按键，经 robot_core 分发调用；运行态 K3 按下沿另经 `RobotCore_ConfirmTask()` 转发给题目的 `onConfirm` 钩子（当前题目三、四、五登记）。
- **参数隔离**：同一算法可以在不同任务复用，但每个任务必须拥有独立可调参数宏和 profile，禁止跨任务直接共用增益、死区、滤波、静摩擦、速度或保持条件。任务三使用 `T3_*`（含钢珠闭环的 `T3_BALL_*`），任务四使用 `T4_*`/`T4_BALL_*`，任务五使用 `T5_*`/`T5_BALL_*`；三题参数均按业务需要独立定义，数值互不影响；新增任务必须在自身 `taskN.c` 另建参数组。
...
- **转向输出软件限速**（题目四、五各自独立）：`Task5_UpdateTracking()` 里 `Pid_Update()` 算出的原始转向量存入 `s_lastSteerRpm` 后，只作为"目标"；`s_appliedSteerRpm` 每拍最多向该目标靠近 `T5_STEER_SLEW_RPM_PER_SEC×T5_DT_SEC`，`Task5_ApplyNormalTracking()`/`Task5_ApplyDecelTracking()` 计算左右轮差速时改用限速后的 `s_appliedSteerRpm`（不再直接用 `s_lastSteerRpm`）。用于抑制离散灰度传感器命中路数切换带来的台阶式转向量一拍内跳到接近 `T5_MAX_STEER_RPM`、车身瞬间大幅横摆把摆杆上的钢珠带得晃动（2026-08 实车反馈"过弯 P 偏大、横摆导致球晃"后新增）。判定到达终点转入 `T5_STATE_AFTER_LINE` 时 `s_lastSteerRpm`/`s_appliedSteerRpm` 都清零，缓停阶段重新进入循迹后从 0 平滑爬升，不带旧转向残留。任务四同样用独立 `T4_STEER_SLEW_RPM_PER_SEC` 实现该限速，且不影响其定时缓停流程；两个限速参数均需按各自实车横摆和过弯跟随情况调节。
...
- **终点判定**（与题目二/四系不同）：先连续 `T5_ARM_TICKS` 拍命中 `≤T5_ARM_HIT_MAX` 路细线，确认已离开出发线才"武装"；武装后单拍命中 `≥T5_STOP_LINE_HIT_MIN`（**当前 5，即 8 路里的 5 路**，用户本次明确给出的新阈值，区别于题目二/四系的 6 路）**且发车（第二次 K3）后累计时间 `≥T5_FINISH_MIN_ELAPSED_MS`（2026-08 新增，当前 3000ms，用原始 tick 计数不叠加 `T5_STOPWATCH_CAL_SCALE` 显示校准）**才判定到达终点，记录当前左右轮平均转速为 `s_straightRpm`，转入 `T5_STATE_AFTER_LINE`。**时间下限的动机**：赛道是环形，起跑线和终点线是同一根线，仅凭"武装+命中路数"防不住发车不久又经过这根线被误判成终点；题目二 `T2_FINISH_MIN_ELAPSED_MS` 使用同一类时间下限且参数独立。
...
- 上述 ID1 位置模式冒烟测试流程已由下列手动定点循迹流程替代，不再执行。

**题目六当前流程（2026-08 手动指定钢珠目标点）**：
...
- **只做登记 + 分发**：`app_robot_core.c` 里的 **dispatch 表 `s_robotTasks[]`** 登记 6 道题的 name + `OnEnter/OnLoop/OnExit` 函数指针；**各题业务代码在 [`../app/tasks/`](../app/tasks/) 的 `taskN.c`**（第 N 题 = `taskN.c`，每题一套状态机；当前 task1 为约 5 秒正常画面录像，task2 为正式循迹，task3 为限位回零并抬升后自动以 X=350 启动闭环、等待 K3 再启动的单次左右摆球，task4 为两段式 K3 启动的钢珠平衡+循迹定时缓停，task5 按说明实现，task6 `ID1 POS` 为 Emm42 ID1 位置模式 API 冒烟测试）。
