...
- **终点判定**（与题目二/四系不同）：先连续 `T5_ARM_TICKS` 拍命中 `≤T5_ARM_HIT_MAX` 路细线，确认已离开出发线才"武装"；武装后单拍命中 `≥T5_STOP_LINE_HIT_MIN`（**当前 5，即 8 路里的 5 路**，用户本次明确给出的新阈值，区别于题目二/四系的 6 路）即判定到达终点，记录当前左右轮平均转速为 `s_straightRpm`，转入 `T5_STATE_AFTER_LINE`。
