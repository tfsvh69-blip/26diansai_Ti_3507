...
- 钢球位置闭环：独立 `BALLCTRL` 线程上电默认 OFF，由题目三/四经 profile 接口按需启停目标（菜单已不提供 K4 启停入口，见下方 ID1 归零限位开关说明），经 α-β + 局部 PD 控制 Emm42 ID1。**2026-07-31 已由速度模式切到位置模式**：`targetPulse = LEVEL_TRIM_PULSE + SIGN*(Kx*error − Kv*velocity)`，每帧直接下发绝对目标脉冲，v1 不做任何输出限幅、I 项或深度保护。位置原点只在本次上电后第一次启动时清零，之后反复进出题目三/四调参沿用同一原点。单帧视觉丢失不再下发任何停止命令（位置模式没有新目标时电机保持原地即安全）；连续两帧 NA 或 220ms 无有效 X 只标记 OLED 状态为 `B:LOST`，恢复需连续两帧有效 X；唯一仍主动下发命令的保护是画面边缘保护。调试流程见 `docs/BALL_CONTROL.md`。
...
- ID1 归零限位开关（HOME_SWITCH）：**PA24**(PINCM54，接口 P1-3)，2026-08-01 由继电器改接，普通 GPIO 数字输入 + 内部上拉，轻触开关一端接地、按下=低电平。封装 `bsp/bsp_home_switch.h`：`BspHomeSwitch_IsPressed()`。`App_Init()` 在 `Emm42Robot_Init()` 之后、任务创建之前调用 `app/app_lift_homing.c` 的 `AppLiftHoming_RunAtBoot()`（`APP_FEATURE_LIFT_HOMING` 门控）驱动 ID1：正方向逼近开关→压下瞬间反向退让至释放（物理归零参考点）→继续负方向移动 `LIFT_HOMING_TARGET_OFFSET_PULSES`（占位待实测）个脉冲到工作位置；开机时开关已被压住则跳过逼近直接退让。取代了此前"每次上电先人工把杆摆到目视水平"的步骤；继电器相关代码（`bsp_relay.c/h`、`app_relay_test_task.c/h`、`APP_FEATURE_RELAY`/`APP_FEATURE_RELAY_SELFTEST`、OLED 状态栏 `R:ON/OFF`）已整体删除，原接线风险 R7（PA24 上电前高阻可能误吸合）随之解除。
...
- 张大头 Emm42_V5.0 闭环步进（UART1）：PA17=TX(→驱动器RX)/PB5=RX(←驱动器TX)，MFCLK 4MHz + 16x 过采样，115200 8N1，v1.1 排针 H7。**2026-07-29 起总线上挂 3 台设备**，靠设备地址(1/2/3)区分：地址1=摆杆高低调节(LIFT)、地址2=左轮(WHEEL_L)、地址3=右轮(WHEEL_R)。与 TMC2209 开环 STEP/DIR 完全不同：这是**串口命令式闭环驱动**，MCU 只发命令帧、不产生脉冲、不占定时器，速度/位置由驱动器内部执行。**题目二、题目三、题目四与题目五使用此系统；4 路 TMC2209 开环电机（bsp_motor）保留给独立测试。**
  - **两层 API**：`module/emm42/emm42_v5.h` 是纯协议层，提供 `Emm42_Enable/SetSpeedRpm/VelControl/PosControl/StopNow/SyncMotion/ReadSysParams` 及诊断计数（`Emm42_GetRxByteCount` 判断总线是否通），只认设备地址、不知道地址对应哪个部件；`module/emm42/emm42_robot.h` 在其上按本车实际用途包一层角色映射（`Emm42RobotId_t`: `EMM42_ROBOT_LIFT/WHEEL_L/WHEEL_R`），提供 `Emm42Robot_Init/Enable/SetSpeedRpm/VelControl/MoveRelative/MoveAbsolute/ResetPosToZero/ClearClogProtection/Stop/StopAll/GetAddr`，业务代码应优先用这层，不直接碰地址数字。
  - **位置模式（摆杆控制的当前形态）**：`Emm42Robot_MoveAbsolute()` 每帧下发"摆杆该停在哪个绝对脉冲位置"，把控制量从角速度降回角度，系统由三阶降为球杆系统标准的二阶 PD 外环。原点由 `Emm42Robot_ResetPosToZero()` 建立；2026-08-01 起摆杆已加 PA24 归零限位开关，开机由 `AppLiftHoming_RunAtBoot()` 自动归零，不再需要人工摆平（见上方「ID1 归零限位开关」段落）。⚠️ `MoveAbsolute` 不能照抄 `MoveRelative` 的 `pulses==0` 早退——绝对模式下 0 是回原点的常用目标。题目六 `ID1 POS` 先做该 API 的冒烟测试（已实测 10 圈=80mm 行程），`BALLCTRL` 随后正式切换为位置模式；手册结论与踩坑清单见 `docs/BALL_CONTROL.md` §10。
  ...
  - 所有接口不等待回复；调度器运行后，协议出口用互斥量保证多线程整帧原子并统一保留至少 6ms 帧间隔。题目二、四、五仍按 30ms 轮询拍最多发一帧；`BALLCTRL` 只在新视觉帧到达时更新一次绝对目标，单帧丢球不再触发任何额外命令。回复走 UART1 RX 中断（无独立任务），以校验字节 `0x6B` 分帧，仅用于诊断不解析字段。
...
- `APP_FEATURE_BALL_CONTROL`（默认 1）：创建 `BALLCTRL` 后台线程，但上电不使能 ID1；依赖 `VISION_LINK` 和 `EMM42`，编译期护栏禁止缺少依赖的组合。
- `APP_FEATURE_EMM42`（默认 1）：张大头 Emm42_V5.0 闭环步进。它自身不创建任务，`App_Init` 先调 `Emm42Robot_Init()` 注册 UART1 回复接收中断，再按 `APP_EMM42_BOOT_DISABLE_RETRY_COUNT=3` 对 ID1/ID2/ID3 循环下发失能帧；每帧后用 `APP_EMM42_BOOT_DISABLE_GAP_MS=10ms` 裸机延时，确保 MCU 复位后全部失能。协议出口现用互斥量保证多线程整帧原子，并统一留至少 6ms 帧间隔。命令由题目或 `BALLCTRL` 经 `Emm42Robot_*` 角色化接口下发。
- `APP_FEATURE_LIFT_HOMING`（默认 1，依赖 `EMM42`，2026-08-01 新增）：开机 ID1 自动归零，`App_Init` 在 `App_Emm42BootDisableAll()` 之后、任何任务创建之前调用 `AppLiftHoming_RunAtBoot()`；取代了原 `APP_FEATURE_RELAY`/`APP_FEATURE_RELAY_SELFTEST`（继电器功能与相关代码已整体删除，PA24 改接归零限位开关）。
